...
`'!a'` \(áp dụng hàm [`ascii()`](https://docs.python.org/3/library/functions.html#ascii)\), `'!s'` \(áp dụng hàm [`str()`](https://docs.python.org/3/library/stdtypes.html#str)\) và `'!r'` \(áp dụng hàm [`repr()`](https://docs.python.org/3/library/functions.html#repr)\) để chuyển đổi giá trị trước khi trước khi in:&gt;&gt;&gt;
